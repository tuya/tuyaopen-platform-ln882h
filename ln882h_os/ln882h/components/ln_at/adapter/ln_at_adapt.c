...
void *ln_at_timer_create(ln_at_timer_type_t type, void *cb, void *arg, uint32_t timeout)
{
    ...
    if (LN_OS_OK != OS_TimerCreate(timer, timer_type, (OS_TimerCallback_t)cb, arg, timeout))
    {
        return NULL;
    }
    return timer;
...
{
    if (LN_OS_OK != OS_TimerStart((OS_Timer_t *)timer))
    {
        return 1;
    }
...
{
    if (LN_OS_OK != OS_TimerChangePeriod((OS_Timer_t *)timer, timeout))
    {
        return 1;
    }
...
{
    if (LN_OS_OK != OS_TimerStop((OS_Timer_t *)timer))
    {
        return 1;
    }
...
{
    if (LN_OS_OK != OS_TimerDelete((OS_Timer_t *)timer))
    {
        return 1;
    }
    LN_AT_FREE(timer);
...
{
    if (LN_OS_OK != OS_ThreadDelete(task))
    {
        return 1;
    }
    return 0;
