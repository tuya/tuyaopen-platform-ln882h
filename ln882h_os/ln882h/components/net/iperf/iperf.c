...
static void iperf_udp_server(void *thread_param)
{
    ...
    uint64_t sentlen;
    int32_t pcount = 0, last_pcount = 0;
        ...
        while (param.mode != IPERF_MODE_STOP) {
            ...
            while ((param.mode != IPERF_MODE_STOP) && ((tick2 - tick1) < (OS_HZ * 5))) {
                ...
                if ((r_size > 0) && (r_size > 200)) {//sizeof(UDP_datagram_t)
                    ...
                    pcount = (int32_t)ntohl(udp_h->id);
                    if (!pcount) last_pcount = 0;
...
int iperf(char *args)
{
    ...
    if (param.mode == IPERF_MODE_STOP) {
        ...
        if (OS_SemaphoreCreateBinary(&param.kill_signal) != LN_OS_OK) {
            IPERF_PRINTF("OS_SemaphoreCreateBinary param.kill_signal fail.\r\n");
            return -1;
        }
        if (use_udp) {
            if (mode == IPERF_MODE_CLIENT) {
                if(LN_OS_OK != OS_ThreadCreate(&(param.iperf_thr), "iperfc", iperf_udp_client, NULL, OS_PRIORITY_HIGH, 6*256)) {
                    IPERF_PRINTF("OS_ThreadCreate g_iperf_thr error!\r\n");
                    return -1;
                }
            } else if (mode == IPERF_MODE_SERVER) {
                if(LN_OS_OK != OS_ThreadCreate(&(param.iperf_thr), "iperfd", iperf_udp_server, NULL, OS_PRIORITY_HIGH, 6*256)) {
                    IPERF_PRINTF("OS_ThreadCreate g_iperf_thr error!\r\n");
                    return -1;
                }
            }
            ...
            if (mode == IPERF_MODE_CLIENT) {
                if(LN_OS_OK != OS_ThreadCreate(&(param.iperf_thr), "iperfc", iperf_client, NULL, OS_PRIORITY_NORMAL, 6*256)) {
                    IPERF_PRINTF("OS_ThreadCreate g_iperf_thr error!\r\n");
                    return -1;
                }
            } else if (mode == IPERF_MODE_SERVER) {
                if(LN_OS_OK != OS_ThreadCreate(&(param.iperf_thr), "iperfd", iperf_server, NULL, OS_PRIORITY_NORMAL, 6*256)) {
                    IPERF_PRINTF("OS_ThreadCreate g_iperf_thr error!\r\n");
                    return -1;
                }
            }
...
int dma_test(char *args)
{
    ...
    if (dma_tx_ctrl.mode == IPERF_MODE_STOP) {
        ...
        if(LN_OS_OK != OS_ThreadCreate(&(dma_tx_ctrl.dma_thr), "dma_thr", dma_tx, NULL, OS_PRIORITY_NORMAL, 4*256)) {
