            ...
            if (FD_ISSET(dhcpd->dhcp_socket, &read_set))
            {
                ...
                if (msg->send_buff == NULL) {
                    if(NULL == (msg->send_buff = OS_Malloc(DHCP_MAX_MTU))) {
                        DHCPD_ERR("[%s, %d]Allocate memory failed!\r\n", __func__, __LINE__);
                        LOG(LOG_LVL_INFO, "malloc error 1!\r\n");
                        continue;
                    }
                }
                ...
                if(rcv_size < 0) {
                    ...
                    LOG(LOG_LVL_INFO, "Receive data error!\r\n");
                    continue;
                }
                DHCPD_PRINTF("%d bytes received\r\n", rcv_size);
                LOG(LOG_LVL_INFO, "%d bytes received\r\n", rcv_size);
...
//    system_parameter_get_dhcpd_config(&(dhcpd->server_config));
    ...
    if (0 != OS_ThreadCreate(&(dhcpd->dhcpd_thr), "dhcpd", dhcpd_task, (void *)dhcpd, OS_PRIORITY_ABOVE_NORMAL, DHCPD_TASK_STACK_SIZE)) {
