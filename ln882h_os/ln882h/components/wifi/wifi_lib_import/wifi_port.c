...
#define SNIFFER_MEM_POOL_CHUNK_CNT         (8) // Office environment maximum use 32
#define SNIFFER_MEM_POOL_CHUNK_BUF_SIZE    (600)
...
wlib_thread_t wlib_os_thread_creat(const char *name, void *entry, void *arg)
{
    ...
    if (LN_OS_OK != OS_ThreadCreate(thread, name, (OS_ThreadEntry_t)entry, arg, priority, stksize)) {
...
wlib_queue_t wlib_os_queue_creat(uint32_t item_count, uint32_t item_size)
{
    ...
    if (LN_OS_OK != OS_QueueCreate(queue, item_count, item_size)) {
...
{
    if (LN_OS_OK != OS_QueueSend((OS_Queue_t *)*pqueue, item, timeout_ms)) {
...
{
    if (LN_OS_OK != OS_QueueReceive((OS_Queue_t *)*pqueue, item, timeout_ms)) {
        return WIFI_ERR_OS_SERVICE;
    }
    return WIFI_ERR_NONE;
}
...
wlib_mutex_t wlib_os_mutex_creat(void)
{
    ...
    if (LN_OS_OK != OS_MutexCreate(mutex)) {
...
{
    if (LN_OS_OK != OS_MutexLock((OS_Mutex_t *)*pmutex, timeout_ms)) {
        return WIFI_ERR_OS_SERVICE;
    }
    return WIFI_ERR_NONE;
}
...
{
    if (LN_OS_OK != OS_MutexUnlock((OS_Mutex_t *)*pmutex)) {
        return WIFI_ERR_OS_SERVICE;
    }
    return WIFI_ERR_NONE;
}
...
wlib_sem_t wlib_os_sem_create(uint32_t max_count, uint32_t init_count)
{
    ...
    if (LN_OS_OK != OS_SemaphoreCreate(sem, init_count, max_count)) {
...
wlib_sem_t wlib_os_sem_binary_create(void)
{
    ...
    if (LN_OS_OK != OS_SemaphoreCreateBinary(sem)) {
...
{
    if (LN_OS_OK != OS_SemaphoreWait((OS_Semaphore_t *)*psem, timeout_ms)) {
        return WIFI_ERR_OS_SERVICE;
    }
    return WIFI_ERR_NONE;
}
...
{
    if (LN_OS_OK != OS_SemaphoreRelease((OS_Semaphore_t *)*psem)) {
        return WIFI_ERR_OS_SERVICE;
    }
    return WIFI_ERR_NONE;
}
...
wlib_timer_t wlib_os_timer_creat(void *timer_cb, uint32_t data)
{
    ...
    if (LN_OS_OK != OS_TimerCreate(timer, OS_TIMER_ONCE, (OS_TimerCallback_t)timer_cb, (void *)data, 200)) {
