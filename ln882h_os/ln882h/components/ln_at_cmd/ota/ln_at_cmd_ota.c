...
static ln_at_err_t ln_at_set_http_ota(uint8_t para_num, const char *name)
{
    ...
    if (LN_OS_OK != OS_ThreadCreate(&g_httpota_thread, "http_ota", http_ota_entry, NULL, OS_PRIORITY_NORMAL, HTTP_OTA_DEMO_STACK_SIZE))
    {
        goto __exit;
    }
...
LN_AT_CMD_REG(START_HTTPC_OTA, NULL, ln_at_set_http_ota, ln_at_help_http_ota, NULL);
