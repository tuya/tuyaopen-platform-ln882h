 ...
 *                     in the queue.
 * @retval OS_Status, LN_OS_OK on success
 */
...
OS_Status OS_QueueSendToFront(OS_Queue_t *queue, const void *item, OS_Time_t waitMS)
{
	BaseType_t ret;
	BaseType_t taskWoken;
	...
	{
		taskWoken = pdFALSE;
		ret = xQueueSendToFrontFromISR(queue->handle, item, &taskWoken);
		if (ret != pdPASS) {
			OS_DBG("%s() fail @ %d\r\n", __func__, __LINE__);
			return LN_OS_FAIL;
		}
	...
	{
		ret = xQueueSendToFront(queue->handle, item, OS_CalcWaitTicks(waitMS));
		if (ret != pdPASS) {
			OS_DBG("%s() fail @ %d, %"OS_TIME_F" ms\r\n", __func__, __LINE__, waitMS);
			return LN_OS_FAIL;
		}
...
int OS_QueueMessagesWaiting(OS_Queue_t *queue)
{
    OS_HANDLE_ASSERT(OS_QueueIsValid(queue), queue->handle);

	if (OS_IsISRContext())
	{
        return uxQueueMessagesWaitingFromISR(queue->handle);
	}
	else
	{
        return uxQueueMessagesWaiting(queue->handle);
	}
}
