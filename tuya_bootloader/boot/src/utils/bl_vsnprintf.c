/****************************************************************************
 * @file bl_vs.c
 * @brief this module is used to bl_vs
 * @version 0.0.1
 * @date 2022-06-15
 *
...
static double_with_bit_access get_bit_access(double x)
{
...
static int get_sign(double x)
{
  // The sign is stored in the highest bit
  return get_bit_access(x).U >> (DOUBLE_SIZE_IN_BITS - 1);
}

static int get_exp2(double_with_bit_access x)
{
...
static void out_discard(char character, void* buffer, size_t idx, size_t maxlen)
{
...
static unsigned int strnlen_s_(const char* str, size_t maxsize)
{
  const char* s;
