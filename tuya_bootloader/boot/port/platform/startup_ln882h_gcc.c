 ...
 *----------------------------------------------------------------------------*/
extern const pFunc __Vectors[100];
const pFunc __Vectors[100] __attribute__ ((section(".vectors"))) = {
...
void Reset_Handler(void) {
     ...
     */
    pDest = (uint8_t *)&__bss_ram0_start__;
    for ( ; pDest < (uint8_t *)&__bss_ram0_end__ ; ) {
        *pDest++ = 0UL;
    }
